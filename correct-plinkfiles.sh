...
dataset_dir=/home/anbennett2/scratch/datasets/processed_data/dbgap/WHI/WHIMS_dataset
dataset=WHIMS
...
gawk 'BEGIN{FS="\t"; OFS="\t"}{print $2}' "${dataset_dir}"/"${dataset}"_plink_temp1.bim | gawk 'BEGIN{FS=":";OFS="\t"}{print $1,$2}'  > "${dataset_dir}"/"${dataset}"_plink_temp1_chrpos.bim


paste "${dataset_dir}"/"${dataset}"_plink__temp1*.bim "${dataset_dir}"/"${dataset}"_plink_temp1_chrpos.bim > "${dataset_dir}"/"${dataset}"_plink_temp1_w_chrpos.bim
...
gawk 'BEGIN{FS="\t";OFS="\t"}{print $7,$2,$3,$8,$5, $6}' "${dataset_dir}"/"${dataset}"_plink_temp1_w_chrpos.bim  > "${dataset_dir}"/"${dataset}"_plink_temp1_updated.bim


echo "Copying other files in the set for plink compatibility..."
cp "${dataset_dir}"/"${dataset}"_temp1.bed "${dataset_dir}"/"${dataset}"_temp1_updated.bed
cp  "${dataset_dir}"/"${dataset}"_temp1.fam "${dataset_dir}"/"${dataset}"_temp1_updated.fam
echo "Done"
